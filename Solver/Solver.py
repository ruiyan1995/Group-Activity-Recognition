...
class Solver:
	...
	def __init__(self, data_loaders, dataset_sizes, net, solver_confs):
		# data args
		...
		self.batch_size = solver_confs.batch_size
		self.label_types = solver_confs.label_types
		self.save_prefix = os.path.join('/home/ubuntu/GAR/weights', solver_confs.dataset_name, solver_confs.stage, '_'.join(self.label_types))
		print self.save_prefix

	def training(self, inputs, labels, phase):
		# Get the inputs, and wrap them in Variable
		inputs = {label_type: Variable(inputs[label_type], requires_grad=False) for label_type in self.label_types}
		labels = {label_type: Variable(labels[label_type], requires_grad=False) for label_type in self.label_types}
		...
		if phase == 'trainval':
			loss.backward()
			self.optimizer.step()
		return preds, loss.data[0] # ? Why is [0]? I need to find source code of '_WeightLoss'
		...
		for epoch in range(self.num_epochs):
			...
			for phase in ['trainval', 'test']:
				...
				for data in self.data_loaders[phase]:
					# get the inputs
					inputs = {label_type: data[label_type][0].float() for label_type in self.label_types}
					labels = {label_type: data[label_type][1] for label_type in self.label_types}
					if self.gpu:
				...
				if phase == 'test' and epoch_acc > best_acc:
					...
					torch.save(self.net.state_dict(), self.save_prefix+'.pkl') # deep copy the model
